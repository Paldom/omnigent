...
class BotsSite:
    ...
    def _approval_card(self, request: Any, now: int) -> str:
        ...
        options = "".join(
            ...
            f' value="{_esc(option)}">{_esc(option)}</button>'
        ...
        return (
            ...
            f"{options}"
            '<button class="btn" name="choice" value="__deny__">deny</button></form>'
            f'<p class="foot">approval {_esc(request.id[:12])} · '
    ...
    def verdict(self, form: dict[str, list[str]], *, now: int) -> tuple[str, str]:
        ...
        if request is None:
            return "", f"no approval matching {approval_id!r}"

        approved = choice != "__deny__"
        run = self.store.get_run(request.run_id)
        try:
            command = self.approvals.decide(
                request,
                approved=approved,
                decided_by="human:web",
                now=now,
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if route.path == "/api/bots":
            ...
            self._send(
                ...
                self.site.index(
                    now=now,
                    notice=_first(params, "ok"),
                    problem=_first(params, "err"),
                ),
    ...
    def do_POST(self) -> None:
        ...
        query = f"ok={_q(notice)}" if notice else f"err={_q(problem)}"
        self.send_response(303)
        self.send_header("Location", f"/bots?{query}")
        self.end_headers()
...
def serve(
    ...
    *,
    ...
    There is no authentication here, deliberately and visibly: the answer is
    the tailnet, not a login form nobody would maintain. Binding anything other
    than loopback says so out loud.
    ...
    if host not in ("127.0.0.1", "localhost", "::1"):
        _logger.warning(
            "serving the bots page on %s:%d with no authentication — anyone who can "
            "reach that address can answer approvals. Bind the tailnet address, "
            "never a public one.",
            host,
            port,
        )
